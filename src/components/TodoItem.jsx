  ...
  return (
    <form onSubmit={handleSubmit} className='px-2'>
      <label className='mb-1'>
        <input type='text' value={title} onChange={handleChange} autoFocus className='w-full pl-2 rounded-md text-black dark:text-white dark:bg-slate-500'/>
      </label>
  ...
  return (
    <li className='border-2 border-gray-500 rounded-e flex flex-row justify-between bg-cyan-100 dark:bg-slate-700 '>
      ...
      <div className='flex flex-col border-l-2 border-gray-500 w-1/3'>
        <button className='p-1.5 text-black dark:text-white lg:text-lg' onClick={() => setEditMode(!editMode)}>{editMode?'Cancel':'Edit'}</button>
        <hr className='border-1 border-gray-500'/>
        <button className='p-1.5 text-black dark:text-white lg:text-lg' onClick={() => removeTodo(todo.id)}>Delete</button>
      </div>
    </li>
  );
};
